...
public class ThreadPoolExecutor extends AbstractExecutorService {
     ...
     * the worker count is adjusted accordingly（相应的）, and, if possible, a
     * new worker is created and started, running firstTask as its
     * first task. This method returns false if the pool is stopped or
     * eligible（合格） to shut down. It also returns false if the thread
     ...
     * (in method execute()) to bypass（分流） queuing when there are fewer
     * than corePoolSize threads (in which case we always start one),
     * or when the queue is full (in which case we must bypass queue).
     * Initially（原来） idle（空闲） threads are usually created via（通过）
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        for (;;) {
            ...
            // rs >= SHUTDOWN,并非RUNNING状态
            if (rs >= SHUTDOWN &&
                    // rs == SHUTDOWN -- 线程池为SHUTDOWN的状态，不允许添加
                    ! (rs == SHUTDOWN &&
                            // Runnable为空
                            firstTask == null &&
                            // 阻塞队列不为空
                            ! workQueue.isEmpty()))
                // rs == SHUTDOWN 且 firstTask == null，并且workQueue.isEmpty() == false，不允许添加线程，
                // 因为firstTask == null是为了添加一个没有任务的线程然后再从workQueue中获取任务的，
                // 如果workQueue.isEmpty() == true，则说明添加的任务没有任何意义。

                // !() 中括号内 的语义是当前的线程池状态处于一个SHUTDOWN状态且当前阻塞队列不为空，利用一个
                // 空任务的Worker来处理当前阻塞队列中的任务
                return false;
            ...
            for (;;) {
                ...
                // CAS add worker 失败，再次读取ctl，说明状态发生了变化，重新来一次
                c = ctl.get();
        ...
        try {
            ...
            if (t != null) {
                ...
                try {
                    ...
                    // 或者线程处于SHUTDOWN状态
                    // firstTask == null是为了添加一个没有任务的线程然后再从workQueue中获取任务的 重点 ........
                    if (rs < SHUTDOWN ||
                        ...
                        // 如果启动失败的话且Worker不为空的话，会从HashSet当中移除
                        workers.add(w);

                        // 设置最大的池大小largestPoolSize，workerAdded设置为true
                        int s = workers.size();
                        // largestPoolSize并没有被附上初值
            ...
            if (! workerStarted)
                // 需要做如下几件事
                // workers.remove(w); -- 如果worker不为空的话
    ...
    private Runnable getTask() {
        ...
        for (;;) {
            ...
            // 判断是否需要超时控制，在允许核心线程超时的情况下或者当前线程数量大于核心线程数量
            ...
            try {
                ...
                // timed == true，调用poll()方法，如果在keepAliveTime时间内还没有获取task的话，则返回null，继续循环。
